...
if exist 3rdparty\build (
    ...
    if not exist .\3rdparty\build\opencv.zip (
        powershell -command "wget https://download.3divi.com/open_source_sdk/archives/artifacts/opencv/POS_SDK/3-1-0/opencv-windows-msvc-vc14-x86-64-install-dir.zip -O .\3rdparty\build\opencv.zip"
    )
    ...
    if not exist .\3rdparty\build\pthread.zip (
        powershell -command "wget https://download.3divi.com/open_source_sdk/archives/artifacts/pthread/pthreads_windows_msvc_vc14_install_dir.zip -O .\3rdparty\build\pthread.zip"
    )
